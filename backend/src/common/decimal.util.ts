...
export function roundMoney(value: MoneyInput): number {
  return dec(value).toDecimalPlaces(MONEY_SCALE, MONEY_ROUNDING).toNumber();
}

export function roundUnitCost(value: MoneyInput): number {
  return dec(value).toDecimalPlaces(UNIT_COST_SCALE, MONEY_ROUNDING).toNumber();
}

export function sumMoney(values: MoneyInput[]): Prisma.Decimal {
  return values.reduce<Prisma.Decimal>(
    (sum, value) =>
      sum.plus(dec(value).toDecimalPlaces(MONEY_SCALE, MONEY_ROUNDING)),
...
export function isBalancedMoney(
  ...
  return dec(debits)
    ...
    .equals(dec(credits).toDecimalPlaces(MONEY_SCALE, MONEY_ROUNDING));
}
