      ...
      <AnimatedPage className="max-w-[1600px] w-full mx-auto space-y-6 h-full flex flex-col">
        <div className="flex justify-between items-end shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage production orders, recipes, and raw material processing.</p>
          </div>
        </div>
        ...
        <div className="flex space-x-2 bg-slate-100 dark:bg-slate-800/50 p-1 rounded-xl w-fit overflow-x-auto shrink-0">
          {tabs.filter(t => t.roles.includes(role || '')).map(tab => {
            const isActive = pathname === tab.path || (tab.path !== '/kitchen' && pathname.startsWith(tab.path))
            return (
              <Link
                key={tab.path}
                href={tab.path}
