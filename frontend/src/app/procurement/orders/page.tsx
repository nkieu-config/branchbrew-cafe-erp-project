...
export default function ProcurementPage() {
  ...
  const handleCreateSubmit = async (values: PurchaseOrder) => {
    if (!activeBranchId) {
    ...
    try {
      await createMutation.mutateAsync({
        ...
        items: values.items.map((i: { ingredientId: number, quantity: number, price: number }) => ({
  ...
  const expandedRowRender = (record: PurchaseOrder) => {
    const itemColumns = [
      ...
      { title: 'Quantity Req.', dataIndex: 'quantityRequested', key: 'qty', render: (val: number, rec: PurchaseOrderItem & { ingredient: Ingredient }) => `${val} ${rec.ingredient.unit}` },
