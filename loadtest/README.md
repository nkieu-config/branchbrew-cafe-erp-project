# Load test — POS checkout and outbox lag
...
2. **Outbox lag** — how long the *asynchronous* side effects take afterwards: the journal entry, the loyalty points, and the kitchen-display push. This is the number that says what "the ledger trails operations by a moment" actually means in seconds.
...
RATE=1 DURATION=60s npm run loadtest
```
...
| `loadtest/target.json` | Written by the stock script: which product and branch to order |
